...
app.post('/api/reviews', (req, res) => {
  ...
  const newReview = {
    "id": id || `rev-${crypto.randomUUID().replace(/-/g, '').slice(0, 8)}`,
    "name": name,
...
app.delete('/api/reviews/:id', (req, res) => {
  ...
  // Track deleted review IDs on server
  const deletedIds = loadJson('deleted_reviews.json', []);
  if (!deletedIds.includes(reviewId)) {
    deletedIds.push(reviewId);
    saveJson('deleted_reviews.json', deletedIds);
  }

  res.json({ success: true });
});

app.get('/api/deleted-reviews', (req, res) => {
  const deletedIds = loadJson('deleted_reviews.json', []);
  res.json(deletedIds);
});
